...
const DEFAULT_RULES = {
  ...
  port_8610: { enabled: true, consecutive: 1 },
};
const DEFAULT_ACTION_DEFINITIONS = [
  ...
  {
    action_key: 'restart_xbridge', name: '重启 xbridge', display_name: '重启 xbridge', category: 'safe', description: '重启 xvpn-bridge-server 服务并校验 8610 端口',
    ...
    risk_level: 'safe', timeout_seconds: 120, executor_type: 'agent', cooldown_seconds: 1800, max_retries: 1, auto_enabled: 1, requires_approval: 0, batch_enabled: 1,
    trigger_faults: JSON.stringify(['port_8610_down']), success_criteria: JSON.stringify({ services_active: ['xvpn-bridge-server'], ports_up: [8610] }), fallback_action_key: 'update_xbridge', priority: 10, metadata: JSON.stringify({})
  },
  {
  ...
  // === 新增：全量安装 ===
  {
    ...
    trigger_faults: JSON.stringify(['port_8888_down', 'port_443_down', 'component_missing']), success_criteria: JSON.stringify({ services_active: ['xagent', 'xvpn-bridge-server'], ports_up: [8888, 8610] }), fallback_action_key: '', priority: 100, metadata: JSON.stringify({})
  },
  // === 新增：5个独立更新脚本 ===
  {
  ...
  },
  {
    ...
    risk_level: 'guarded', timeout_seconds: 300, executor_type: 'agent', cooldown_seconds: 3600, max_retries: 1, auto_enabled: 0, requires_approval: 0, batch_enabled: 1,
    trigger_faults: JSON.stringify(['port_8610_down']), success_criteria: JSON.stringify({ services_active: ['xvpn-bridge-server'], ports_up: [8610] }), fallback_action_key: '', priority: 15, metadata: JSON.stringify({})
  },
  {
  ...
  },
  {
    ...
    trigger_faults: JSON.stringify([]), success_criteria: JSON.stringify({ services_active: ['xvpn-bridge-server'], ports_up: [8610] }), fallback_action_key: '', priority: 200, metadata: JSON.stringify({ download_url: `${DOWNLOAD_BASE_URL}/packages/xbridge/stable/current/xbridge-server.zip` })
  },
  {
...
function faultTitle(serverId, faultType) { const map = { port_443_down: '内核端口 443 异常', port_6379_down: 'Redis 端口 6379 异常', port_8888_down: 'XAgent 端口 8888 异常', port_8610_down: 'XBridge 端口 8610 异常', server_offline: '节点离线' }; return `${serverId} · ${map[faultType] || faultType}`; }
function suggestAction(faultType) { const map = { port_443_down: 'update_xray', port_6379_down: 'restart_redis', port_8888_down: 'restart_xagent', port_8610_down: 'restart_xbridge', server_offline: '' }; return map[faultType] || ''; }
function upsertIncidentsForServer(serverId, serverStatus, issues, metadata = {}) {
  ...
  for (const issue of issues || []) {
    ...
    else if (String(issue).includes('端口 8610')) faultType = 'port_8610_down';
...
app.post('/api/agent/register', (req, res) => {
  ...
  db.prepare(`INSERT INTO metrics (server_id, cpu_usage, memory_usage, memory_used, memory_total, disk_usage, disk_used, disk_total, port_443, port_6379, port_8888, port_8610, issues) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '[]')`).run(serverId, Number(body.cpu_usage || 0), Number(body.memory_usage || 0), Number(body.memory_used || 0), Number(body.memory_total || 0), Number(body.disk_usage || 0), Number(body.disk_used || 0), Number(body.disk_total || 0), body.ports?.['443'] ? 1 : 0, body.ports?.['6379'] ? 1 : 0, body.ports?.['8888'] ? 1 : 0, body.ports?.['8610'] ? 1 : 0);
