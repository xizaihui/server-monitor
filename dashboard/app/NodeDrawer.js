...
export default function NodeDrawer({ server, onClose, onCopy, onViewIncidents }) {
  if (!server) return null;
  const ports = [['xray / 443', server.port_443], ['redis / 6379', server.port_6379], ['xagent / 8888', server.port_8888], ['xbridge / 8789', server.port_8789]];
  return (
    <div className="drawerOverlay" onClick={onClose}>
      <aside className="drawerPanel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="drawerSection"><div className="drawerSectionTitle">基本信息</div><div className="drawerList"><CopyRow label="IP" value={server.ip} onCopy={onCopy} /><CopyRow label="节点名" value={server.display_name || server.hostname} onCopy={onCopy} /><CopyRow label="Server ID" value={server.server_id} onCopy={onCopy} /><CopyRow label="业务 server-id" value={server.instance_id} onCopy={onCopy} /><div className="drawerRow"><span>分类</span><strong>{server.group_name || '未分组'}</strong></div><div className="drawerRow"><span>状态</span><strong>{server.status}</strong></div><div className="drawerRow"><span>Agent 版本</span><strong>{server.metadata?.agent_version || '-'}</strong></div><div className="drawerRow"><span>脚本版本</span><strong>{server.metadata?.ops_scripts_version || '未初始化'}</strong></div><div className="drawerRow"><span>系统 / 架构</span><strong>{server.os || '-'} / {server.arch || '-'}</strong></div><div className="drawerRow"><span>最后心跳</span><strong>{formatShanghai(server.last_seen)}</strong></div><div className="drawerRow"><span>离线时长</span><strong>{server.status === 'offline' ? formatDuration(server.offline_seconds) : '-'}</strong></div><div className="drawerRow"><span>创建时间</span><strong>{formatShanghai(server.created_at)}</strong></div><div className="drawerRow"><span>更新时间</span><strong>{formatShanghai(server.updated_at)}</strong></div></div></div>
